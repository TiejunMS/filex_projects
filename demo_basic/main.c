...
/* Define raw disk device driver entry.  */
...
CHAR  local_buffer[30];
    ...
    /* Open the RAM disk.  */
    status =  fx_media_open(&ram_disk, "RAM DISK", _fx_raw_disk_driver, ram_disk_memory, media_memory, sizeof(media_memory));
    ...
    printf("FileX basic demo complete\n");
    exit(0);
}
